      ...
      <header className="bg-white shadow-sm border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-serif text-[#2D2A26] mb-1 md:mb-2">Gestion du Personnel</h1>
              <p className="text-sm md:text-base text-gray-500">Administration de l'équipe et des utilisateurs</p>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-8">
        ...
        {viewMode === 'staff' && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
              ...
              <StatsCard title="Admins" value={staff.filter(s => s.role === 'Admin').length} color="purple" />
              <StatsCard title="Vendeurs" value={staff.filter(s => s.role === 'Vendeur').length} color="gold" />
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Membre</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Contact</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Rôle</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Date d'arrivée</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Statut</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Dernière connexion</th>
                      ...
                      <tr key={member.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold text-[#2D2A26]">{member.fullName}</p>
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2 text-sm text-gray-600">
                            <Calendar size={14} />
                            <span>{new Date(member.joinDate).toLocaleDateString('fr-FR')}</span>
                          </div>
                        </td>
                        <td className="p-4">
              ...
              {filteredStaff.map((member) => (
                <div key={member.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full bg-[#C5A065] text-white flex items-center justify-center font-bold flex-shrink-0">
                        {member.fullName.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div>
                        <h3 className="font-semibold text-[#2D2A26]">{member.fullName}</h3>
                        <RoleBadge role={member.role} />
                      </div>
                    </div>
                    <StatusBadge status={member.status} />
        ...
        {viewMode === 'users' && (
          <div>
            ...
            {/* Stats Cards - Responsive Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
              <StatsCard title="Total Utilisateurs" value={users.length} color="blue" />
            ...
            <div className="hidden lg:block bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Utilisateur</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Contact</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Membre depuis</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Commandes</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Total dépensé</th>
                      <th className="text-left p-4 text-sm font-semibold text-gray-600">Statut</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold text-[#2D2A26]">{user.fullName}</p>
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2 text-sm text-gray-600">
                            <Calendar size={14} />
                            <span>{new Date(user.memberSince).toLocaleDateString('fr-FR')}</span>
                          </div>
                        </td>
                        <td className="p-4">
                          <span className="font-semibold text-[#2D2A26]">{user.totalOrders}</span>
                        </td>
                        <td className="p-4">
                          <span className="font-semibold text-green-600">{user.totalSpent.toFixed(2)} €</span>
                        </td>
                        <td className="p-4">
                ...
                <div key={user.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-[#2D2A26]">{user.fullName}</h3>
                        <p className="text-xs text-gray-500">Membre depuis {new Date(user.memberSince).toLocaleDateString('fr-FR')}</p>
                      </div>
                    </div>
                    <StatusBadge status={user.status} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-100">
                    <div>
                      ...
                      <p className="font-semibold text-[#2D2A26]">{user.totalOrders}</p>
                    </div>
                    <div>
        ...
        {viewMode === 'settings' && (
          <div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-8 max-w-2xl">
              <h3 className="text-lg md:text-xl font-serif text-[#2D2A26] mb-4 md:mb-6">Paramètres de sécurité</h3>
              ...
              <div className="space-y-4">
                <h4 className="font-serif text-base md:text-lg text-[#2D2A26] mb-4">Préférences de notification</h4>
                ...
                <label className="flex items-center justify-between p-3 md:p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
                  <span className="text-sm md:text-base text-gray-700">Notifications par email</span>
                  <input type="checkbox" defaultChecked className="w-5 h-5 text-[#C5A065]" />
                </label>

                <label className="flex items-center justify-between p-3 md:p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
                  <span className="text-sm md:text-base text-gray-700">Alertes de stock bas</span>
                  <input type="checkbox" defaultChecked className="w-5 h-5 text-[#C5A065]" />
                </label>

                <label className="flex items-center justify-between p-3 md:p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors">
                  <span className="text-sm md:text-base text-gray-700">Rapport hebdomadaire</span>
...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span className={`inline-block px-2 md:px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${styles}`}>
      {role}
    </span>
  );
}
...
function StatsCard({ title, value, color }: StatsCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
      <p className="text-xs md:text-sm text-gray-500 mb-2 uppercase tracking-wider font-semibold">{title}</p>
