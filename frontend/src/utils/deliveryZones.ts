// Delivery zones configuration based on postal codes
...
/**
 * Get all postal codes as a formatted string for each zone
 */
export function getFormattedZones(): string[] {
  return DELIVERY_ZONES.map(
    (zone) =>
      `${zone.name}: ${zone.postalCodes.join(", ")} - Livraison: ${zone.deliveryFee.toFixed(2)}$ (Minimum: ${zone.minimumOrder.toFixed(2)}$)`,
  );
}
